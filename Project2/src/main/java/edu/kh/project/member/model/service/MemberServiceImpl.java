...
//
	...
	/*
	 * ** 스프링에서 트랜잭션을 처리하는 방법 ** - 선언적 트랜잭션 처리
	 * 
	 * 1) <tx:advice> 를 이용한 XML 작성 방법
	 * 
	 * 2) @Transactional 어노테이션 작성 방법 - 클래스, 메서드 위에 작성 가능 - 조건 : AOP를 이용한 기술 ->
	 * Service Interface 필요 트랜잭션 매니저가 bean으로 등록되어 있어야 함. (root-context.xml)
	 * 
	 * @Transactional 어노테이션 특징 - 예외가 발생한 경우 rollback 자동 진행 - 예외의 기본값은
	 * RuntimeException -> SQL 진행 시 발생하는 예외 == SQLExcption(RuntimeException 형제) (다형성
	 * 적용 X) -> 다른 예외에도 rollback이 진행되고 싶다면 rollbackFor = 예외클래스 작성하면된다
	 */
